class WhatsAppChatbot {
	...
	async handleIncomingMessage(message) {
		try {
			...
			// Derive real sender: handle @lid by falling back to sender phone when available
			let senderJid = message.key?.remoteJid || '';
			if (senderJid.endsWith('@lid')) {
				senderJid = message.key?.senderPn || message.senderPn || message.key?.participant || senderJid;
			}
			const cleanDigits = String(senderJid).replace(/@.+$/, '').replace(/[^0-9]/g, '');

			this.logger?.info({ phoneNumber: senderJid, text: messageText }, 'received message');

			// Check if user typed "sobha" (case insensitive)
			if (messageText.trim().toLowerCase() === 'sobha') {
				await this.handleSobhaCommand(cleanDigits, cleanDigits);
				return;
			}

			// Check if user is in a conversation (waiting for empid)
			const conversation = this.conversations.get(cleanDigits);
			if (conversation && conversation.stage === 'awaiting_empid') {
				await this.handleEmpidResponse(cleanDigits, cleanDigits, messageText.trim());
				return;
			}
