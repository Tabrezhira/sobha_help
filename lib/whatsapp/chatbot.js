	...
	async handleSobhaCommand(cleanPhone, whatsappNumber) {
		try {
			...
			if (employee && employee.salarySlip) {
				// Employee found with salary slip - send it
				await this.sendSalarySlip(employee, whatsappNumber);
				this.conversations.delete(cleanPhone);
			} else if (employee && !employee.salarySlip) {
				// Employee found but no salary slip
				...
				// Employee not found - ask for empid
				await this.messageService.sendText({
					to: whatsappNumber,
